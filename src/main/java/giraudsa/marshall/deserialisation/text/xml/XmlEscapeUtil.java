...
public final class XmlEscapeUtil {
	...
	public static String unescape(final String text) {
		...
		for (int i = offset; i < max; i++) {
			...
			if (c == REFERENCE_PREFIX) {
				...
				if (c1 == '\u0020' || // SPACE
						...
						c1 == '\u003C' || // LES-THAN SIGN
						c1 == '\u0026')
					// Not a character references. No characters are consumed, and nothing is
					// returned.
					continue;
			...
			/*
			 * At this point we know for sure we will need some kind of unescape, so we can
			 * increase the offset and initialize the string builder if needed, along with
		...
		/*
		 * -----------------------------------------------------------------------------
		 * ------------------ Final cleaning: return the original String object if no
		 * unescape was actually needed. Otherwise append the remaining escaped text to
		 * the string builder and return.
		 * -----------------------------------------------------------------------------
		 * ------------------
		 */
